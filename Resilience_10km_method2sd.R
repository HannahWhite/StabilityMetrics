#################################################################################################
### Calculating resilience metrics using biggest drop window methods at 10 km Scale with 2 sd ###
#################################################################################################
...
load('J:\\Postdoc Grassland Resilience\\MODIS6\\Anomalies_18.02.2019\\evi.anomaly10km.cent.RData')
...
try(
  for (i in 1:dim(eastings)[1]){
    for (j in 1:dim(eastings)[2]){
      ind.sd <- which(anomaly.03[i, j, ]<=-2) # finds anomalies which deviate more than two standard deviations from the zero baseline
      anomaly.sub <- anomaly.03[i,j,ind.sd] # subsets the anomaly dataframe to just those that are greater than two standard deviations
...
#save(evi.all.events, evi.dates, evi.dates.big, evi.events, evi.magnitude, evi.magnitude.median, evi.all.rectime, evi.all.rate, evi.rectime, evi.rate, 
#evi.rectime.median, evi.rate.median, evi.variability, evi.rectime.big, evi.rate.big, evi.longest.rec, evi.slowest.rate,
...
stability10km <- merge(stability10km, evi.long, by = c('eastings', 'northings'), all = TRUE)
stability10km <- merge(stability10km, evi.slow, by = c('eastings', 'northings'), all = TRUE)
...
stability10km$evi.magmed <- 1/abs(stability10km$evi.magmed)
stability10km$evi.drop <- 1/abs(stability10km$evi.drop)


write.csv(stability10km, 'J:\\Postdoc Grassland Resilience\\MODIS6\\Resilience\\stability10km2sd.csv', row.names = FALSE)

cor(stability10km[,3:13], use = 'na.or.complete', method = 'spearman') # use Spearman's as many correlations appear non linear (see pair graph)

pairs(stability10km[,3:13])
