######################################################################
### Analyses and graphics for stability metrics at 1 km resolution ###
######################################################################

### Hannah White 03.04.2019
### Edited 25.04.2019 to add 2sd events to timing of events graphic
### Edited 01.05.2019 to change scale on maps
...
stability.redless$evi.magmed[which(stability.redless$evi.magmed>0.65)] <- NA #changes 194
stability.redless$evi.recmed[which(stability.redless$evi.recmed>60)] <- NA #changes 145
...
tiff('J:\\Postdoc Grassland Resilience\\Writing\\MODIS_resilience_metrics\\Graphics\\maps1km.tiff', height = 1000, width = 1000) #may need to adjust cut off point
par(oma = c(0,0,0,6))
...
p <- p + scale_x_date(breaks = waiver(),
                      ...
                      limits = c(as.Date("2003-01-01"), 
                                 as.Date("2019-01-25")))
p <- p + xlab('')
p <- p + scale_fill_manual(values = c('grey70', 'grey20'))
...
tiff('J:\\Postdoc Grassland Resilience\\Writing\\MODIS_resilience_metrics\\Graphics\\events1kmextra.tiff', width = 14, height = 14, units = 'in', res = 360)
